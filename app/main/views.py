...
def post(id):
	...
	page = request.args.get('page', 1, type=int)
	if page == -1:
		page =(post.comments.count())// current_app.config['COMMENTS_PER_PAGE']+1 #地板除+1表示页码
	pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(page, per_page=current_app.config['COMMENTS_PER_PAGE'])
